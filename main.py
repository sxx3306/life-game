...
class Button(pygame.sprite.Sprite):
    def __init__(self, color: tuple, width: float, height: float, position: tuple, func):
        super().__init__()
...
class Game:
    def __init__(self, grid: tuple[int, int], resolution: tuple[float, float] = (800.0, 600.0), rules: list[list] = [[2, 3], [3]]):
        ...
        self.start_button = Button('GREEN', 100, 20, (self.res[0] - 150, 50), start_button)
        self.clear_button = Button('RED', 100, 20, (self.res[0] - 150, 100), clear_button)
        self.running = False
    ...
    def run(self):
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONUP:
                    if self.start_button.handle_click(event):
                        pass
                    elif self.clear_button.handle_click(event):
                        self.clear_board()
                    else:
                        pos = event.pos
            ...
            self.clear_button.blit(self.screen)
            pygame.display.flip()
            self.clock.tick(60)
...
def clear_button(self) -> np.ndarray:
    return True
